...
const FormularioHabitacion = ({ crear, show, handleClose, habitacionId }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const cargarHabitacion = async (id) => {
    try {
      const datos = await obtenerHabitacionesId(id);
      ...
      setValue("disponible", datos.disponible);
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Error",
  ...
  const onSubmit = async (habitacion) => {
    try {
      ...
      habitacion.disponible =
        habitacion.disponible === "true" || habitacion.disponible === true;
      if (crear) {
            ...
            <Form.Control
              ...
              {...register("imagen", {
                pattern: {
                  ...
                  message:
                    "Debe ingresar una URL de imagen válida (jpg, jpeg, gif, png)",
                },
              })}
            />
            <Form.Text className="text-danger">
